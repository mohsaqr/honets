...
| B1 | Windowed sequence hyperedges: sequence input for `build_hypergraph()` (tumbling/sliding window w, hyperedge = distinct states in window, weight = window count) | `build_hypergraph(data, window = w)` | no upstream oracle — invariants: window-count conservation; w = 2 reduces to pairwise co-occurrence | Ding et al. (2020) EMNLP (HyperGAT construction) |
